...
def main() -> None:
    ...
    global_option = Option(budget=1, env=env, parent_option=None, N=hyperparams['N'], K=hyperparams['K'])
    goal_option = Option(hyperparams['budget'], env=env, parent_option=None, N=hyperparams['N'], K=hyperparams['K'])
    ...
    while not done:
        ...
        if untrained_option.termination_classifier.check(next_obs) and not initial_state_covered(initial_state, option_repertoire):
            ...
            if untrained_option.initiation_classifier_trained:
                untrained_option.agent.load_global_weights(global_option.agent.actor_network, global_option.agent.critic_network)
                # TODO: agent_over_options.add
                option_repertoire.append(untrained_option)
                untrained_option = Option(hyperparams['budget'], env=env, parent_option=untrained_option, N=hyperparams['N'], K=hyperparams['K'])
